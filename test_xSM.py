    ...
    def dilution_factor(self, dt, dphi):
        if self.TcTrans == [] or self.TnTrans == []:
            ...
            if (Tcrit - Tnuc) < 2:
                ...
                f = diffpho / L
                print('f is ', f)
                if f <=1 and L >0 and diffpho >0 and entropy[0] > entropy[1]:
                    if entropy[0] != 1 and  entropy[1] != 1 and entropy[2] != 1:
                        dilution_factor = (1 - f * (entropy[0] - entropy[1]) / entropy[0]) * entropy[0] / (
                            (1 - (entropy[0] - entropy[1]) / entropy[0]) * entropy[2])
                    else:
                        dilution_factor = 444
                        print('entropy is not exist due to inverse of the matrix is not exist')
                        
                else:
                    dilution_factor=999
                    print('f or L or energy difference or S must be not right')
