...
class model(generic_potential.generic_potential):
    ...
    def boson_massSq(self, X, T):
        ...
        if self.RG_onshell_method == True:
            ...
            dof = np.array([1, 1, self.dof_wl, self.dof_wt, self.dof_zl, self.dof_zt, self.dof_zl, self.dof_zt, self.dof_goldstone])  ###do not consider the goldstone now!!
            c = np.array([1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5])  # check Goldstones
        else:
            M = np.array([mh, ms, mwl, mwt, mzl, mzt, mgl, mgt, mx])
            M = np.rollaxis(M, 0, len(M.shape))
            dof = np.array([1, 1, self.dof_wl, self.dof_wt, self.dof_zl, self.dof_zt, self.dof_zl, self.dof_zt, self.dof_goldstone])
    ...
    def select_Tc(self):
        N = len(self.TcTrans)
        for i in range(N):
            if abs(self.TcTrans[i]['high_vev'][0]) < 0.1 and self.TcTrans[i]['trantype'] == 1 and abs(self.TcTrans[i]['low_vev'][0]) > 0.1:
                a = i
                break
        return a
